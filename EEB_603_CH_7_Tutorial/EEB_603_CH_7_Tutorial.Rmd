...
So now that you see the dimensions and a peak at the beginning and end of the data, lets use some others.
...
Now we have a unique ID for each observation and we pulled out only the data we were interested in summarizing.  Now lets organize the data by TaxonClass.
...
Now that we have the taxa group, lets arrange the maximum life expectancy within each taxon. (arrange(first order, second order))
...
Lets change the order from assending MLE to descending using the - sign in front of Overall.MLE
